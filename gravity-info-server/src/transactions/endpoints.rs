        ...
        match item {
            Ok((key, value)) => {
                ...
                if key_parts.len() == 4 && key_parts[1] == "msgSendToEth" {
                    ...
                    let formatted_date = match convert_timestamp(timestamp) {
                        Ok(t) => t,
                        Err(_) => continue,
                    };
...
pub async fn get_all_msg_ibc_recv_transactions(db: web::Data<Arc<DB>>) -> impl Responder {
    let mut response_data: HashMap<u64, BlockData> = HashMap::new();
    ...
    for item in iterator {
        match item {
            Ok((key, value)) => {
                let key_str = String::from_utf8_lossy(&key);
                let key_parts: Vec<&str> = key_str.split(':').collect();
                if key_parts.len() == 4 && key_parts[1] == "msgIbcRecv" {
                    let msg_ibc_transfer: CustomMsgTransfer =
                        serde_json::from_slice(&value).unwrap();
                    let block_number = key_parts[0].parse::<u64>().unwrap();
...
fn convert_timestamp(timestamp: i64) -> Result<String, ()> {
